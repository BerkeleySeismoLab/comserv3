...
begin
  ...
  sprintf((char *)result, "%d-%s-%s %s:%s:%s", nt->wyear, two(nt->wmonth, addr(smth)),
...
begin
  ...
  switch (clock_exception) begin
    case CE_DAILY :
      lib330_padright ((pointer)"Daily Timemark", (pointer)addr(ptim->exception_type), 16) ;
      break ;
    case CE_VALID :
      lib330_padright ((pointer)"Valid Timemark", (pointer)addr(ptim->exception_type), 16) ;
      break ;
    case CE_JUMP :
      lib330_padright ((pointer)"UnExp Timemark", (pointer)addr(ptim->exception_type), 16) ;
      sprintf ((char *)s, "Jump of %s Seconds", (char *)jump_amount) ;
      lib330_padright ((pointer)addr(s), (pointer)addr(ptim->clock_status), 128) ;
      break ;
  end
  strcpy((char *)s, (char *)addr(q330->share.gpsids[4])) ; /* 330 will place clock ID string here for all models */
...
/* next find beginning of string */
  ...
  while ((i > 0) land (s[i] != ' '))
    dec(i) ;
  lib330_padright ((pointer)addr(s[i + 1]), (pointer)addr(ptim->clock_model), 32) ;
...
begin
  ...
  strcat(on_, " ") ;
  sprintf((char *)w, "%s:%s", seed2string((pointer)ppar->location, (pointer)ppar->seedname, (pointer)addr(s2)), pdef->detname) ;
  while (strlen(w) < 18)
    strcat(w, " ") ;
  if (det->det_options and DO_MSG)
    then
      begin
        sprintf(s, "%s-%s", w, on_) ;
        libmsgadd(q330, LIBMSG_DETECT, (pointer)addr(s)) ;
      end
  ...
  if (mh)
    then
      begin
        ...
        lib330_padright ((pointer)addr(pdef->detname), (pointer)addr(eblk.s_detname), 24) ;
      ...
      begin
        ...
        pt->next_blockette = 0 ;
        lib330_padright ((pointer)addr(pdef->detname), (pointer)addr(pt->s_detname), 24) ;
...
begin
  ...
  lib330_padright ((pointer)"3DB@10Hz", (pointer)addr(pc2->rolloff), 12) ;
...
begin
  ...
  switch (chan and not DCM) begin
    case SP_CALSTART :
      ...
      psine = (sine_calibration *)addr(cblk) ; /* common fields */
      ...
      /* white uses random amplitudes, red and telegraph don't */
          ...
          switch (cals.waveform and 7) begin
            case 1 :
              lib330_padright((pointer)"Red", (pointer)addr(prand->noise_type), 8) ;
              break ;
            case 2 :
              lib330_padright((pointer)"White", (pointer)addr(prand->noise_type), 8) ;
              break ;
            case 4 :
              lib330_padright((pointer)"Telegraf", (pointer)addr(prand->noise_type), 8) ;
              break ;
          end
          storerandom (addr(p), prand) ;
          break ;
        case 3 : /* step */
          pstep = (step_calibration *)addr(cblk) ;
    ...
    case SP_CALABORT :
      ...
      map = cals.calbit_map ;
      cals.monitor_map = loadword (addr(pb)) ;
      pabort = (abort_calibration *)addr(cblk) ;
...
begin
  ...
  strcat((char *)msg, "\x0D\x0A") ;
  pc = (pointer)((pntrint)addr(pcom->ring->rec) + 56 + pcom->frame) ; /* add text to data area */
  memcpy(pc, msg, strlen((char *)msg)) ;
  incn(pcom->frame, strlen((char *)msg)) ;
