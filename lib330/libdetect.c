...
begin
  ...
  if (q->raw_data_source == READ_PREV_STREAM)
    then
      begin
        pdp->indatar = (pointer)addr(q->processed_stream) ;
        pdp->singleflag = TRUE ;
      end
    else
      begin
  ...
  if (mh)
    then
      begin
        getbuf (q330, (pointer)addr(pmh), sizeof(con_sto)) ;
        pdp->cont = pmh ;
      ...
      begin
        getbuf (q330, (pointer)addr(pt), sizeof(threshold_control_struc)) ;
        pdp->cont = pt ;
  ...
  if (pi)
    then
      begin
        pdp->indatar = (pointer)addr(pi->out) ;
        pdp->singleflag = TRUE ;
      end
end
    ...
    begin
      ...
      while ((detstat->count < MAX_DETSTAT) land (pd != NIL))
        begin
          ...
          sprintf((char *)addr(pone->name), "%s:%s", seed2string((pointer)q->location, (pointer)q->seedname, (pointer)addr(s)),
  ...
  while (q)
    begin
      ...
      while (pd)
        begin
          pcc = pd->cont ;
          sprintf((char *)s, "%s:%s", seed2string((pointer)q->location, (pointer)q->seedname, (pointer)addr(s)),
                  (char *)addr(pd->detector_def->detname)) ;
          if (strcmp((char *)addr(detchange->name), (char *)addr(s)) == 0)
            then
              begin
