  ...
  if (dssstr->verbosity >= RPT_ALLMEM)
    then
      begin
        ...
        lib_msg_add(dssstr->q330, AUXMSG_DSS, 0, (pointer)addr(s)) ;
      end
  ...
  if (smallpt == NIL)
    then
      begin /* nothing found there */
        ...
        getbuf (dssstr->q330, (pointer)addr(mpt), msize) ;
...
begin
  ...
  if (dssstr->q330->dsspath == INVALID_SOCKET)
    then
      begin
        ...
        lib_msg_add(dssstr->q330, AUXMSG_SOCKETERR, 0, (pointer)addr(s)) ;
        sprintf(s, "DSS Socket Error: %d", err) ;
        strcpy ((char *)addr(dssstr->dss_server_display), (char *)addr(s)) ;
        return ;
      end
#ifndef X86_WIN32
  ...
  if (err)
    ...
    then
      begin
        ...
        dssstr->q330->dsspath = INVALID_SOCKET ;
        sprintf(s, "%d on DSS", err) ;
        lib_msg_add(dssstr->q330, AUXMSG_BINDERR, 0, (pointer)addr(s)) ;
  ...
  lib_msg_add(dssstr->q330, AUXMSG_SOCKETOPEN, 0, (pointer)addr(s)) ;
...
begin
  ...
  dssstr->refused = TRUE ;
  p = (pbyte)addr(dssstr->msgout.buffer) ;
...
begin
  ...
  for (cp = 2 ; cp >= 0 ; cp--)
    begin
      ...
      psock = (pointer)addr(dssstr->sock) ;
      while (pcli)
        begin
...
begin
  ...
  prio = -1 ;
  p = (pbyte)addr(dssstr->msgin.buffer) ;
  loadstring (addr(p), 8, (pointer)addr(pass)) ;
  if (strlen(pass) > 7)
    then
      pass[7] = 0 ;
  lib330_upper((pchar)addr(pass)) ;
  loadstring (addr(p), 250, (pointer)addr(s)) ;
  if (strlen(s) > 79)
    then
      s[79] = 0 ;
  for (i = 0 ; i <= 2 ; i++)
    if (strcmp((char *)addr(pass), (char *)addr(dssstr->passwords[i])) == 0)
      then
        begin
          prio = i ;
          break ;
  ...
  if (pcli == NIL)
    then
      begin /* new client */
        if (dssstr->verbosity >= RPT_CLIENTS)
          then
            begin
              ...
              lib_msg_add(dssstr->q330, AUXMSG_DSS, 0, (pointer)addr(s2)) ;
            end
        ...
        pcli->timer = 0 ;
        strcpy((char *)addr(pcli->name), (char *)s) ;
  ...
  for (i = 0 ; i <= 4 ; i++)
    if (prep->station[i] != ' ')
      then
        ...
        break ;
  seed2string ((pointer)prep->location, (pointer)prep->channel, (pointer)addr(s2)) ;
  sprintf((char *)res, "%s.%s", s, s2) ;
end
...
begin
  ...
  while (pdet)
    begin
      strcpy(s, (char *)addr(pdet->detector_def->detname)) ;
      lib330_upper((pchar)addr(s)) ;
      if (strcmp(s, (char *)addr(dssstr->detname)) == 0)
      then
        break ;
      else
        pdet = pdet->link ;
        ...
        if (req.reqcode == REQ_DET)
          then
            begin
              ...
              lib330_upper((pchar)addr(dssstr->detname)) ;
            end
      ...
      begin
        ...
        if (newreq->reqcode == REQ_DET)
          then
            begin
              strcat(s, ":") ;
              strcat(s, (char *)addr(dssstr->detname)) ;
            end
...
begin
  ...
  dssstr->lastp = (pbyte)((pntrint)pbeg + 2) ; /* point at offset */
end
  ...
  if ((dssstr->sendhdr.datalength + size) > 512)
    then
      begin
        ...
        dssstr->sendhdr.datalength = 0 ;
        dssstr->lastp = NIL ;
        dssstr->pb = (pbyte)addr(dssstr->msgout.buffer) ;
      end
end
...
begin
  ...
  dssstr->sendhdr.datalength = 0 ;
  dssstr->lastp = NIL ;
  dssstr->pb = (pointer)addr(dssstr->msgout.buffer) ;
  while (prep)
    begin
...
begin
  ...
  psock = (struct sockaddr_in *)addr(dssstr->sock) ;
  for (cp = 2 ; cp >= 0 ; cp--)
    begin
      dssstr->client_priority = cp ;
...
begin
  ...
  getbuf (q330a, (pointer)addr(dssstr), sizeof(tdssstr)) ;
  ...
  strcpy ((char *)addr(dssstr->passwords[0]), (char *)addr(dss->low_pass)) ;
  lib330_upper ((char *)addr(dssstr->passwords[0])) ;
  strcpy ((char *)addr(dssstr->passwords[1]), (char *)addr(dss->mid_pass)) ;
  lib330_upper ((char *)addr(dssstr->passwords[1])) ;
  strcpy ((char *)addr(dssstr->passwords[2]), (char *)addr(dss->high_pass)) ;
  lib330_upper ((char *)addr(dssstr->passwords[2])) ;
...
begin
  ...
  if (dssstr)
    then
      strcpy ((char *)result, (char *)addr(dssstr->dss_server_display)) ;
    else
      (*result)[0] = 0 ;
end
