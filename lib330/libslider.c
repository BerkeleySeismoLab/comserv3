...
begin
  ...
  for (i = 0 ; i <= 255 ; i++)
    getthrbuf (q330, (pointer)addr(q330->pkt_bufs[i]), sizeof(tpkt_buf)) ;
end
...
begin
  ...
  libmsgadd(q330, LIBMSG_LINKRST, (pointer)addr(s)) ;
end
...
begin
  ...
  loops = 0 ;
  p = (pbyte)addr(q330->pkt_bufs[pktidx]->buf.qdp) ;
  ...
  if (lth > MAXDATA)
    then
      begin
        sprintf(s, "%d", lth) ;
        libmsgadd (q330, LIBMSG_INVLTH, (pointer)addr(s)) ;
        return ;
      end
      ...
      if (dsn != (q330->lastfill + 1))
        then
          begin
            ...
            libmsgadd(q330, LIBMSG_FILLJMP, (pointer)addr(s)) ;
          end
      q330->lastfill = dsn ;
      add_status (q330, AC_FILL, 1) ;
      break ;
    case DT_DATA :
      ...
      if ((dsn != paqs->dt_data_sequence) land (dsn != (paqs->dt_data_sequence + 1)))
          ...
          begin
            if (paqs->dt_data_sequence == 0)
              then
                begin
                  sprintf(s, "%d", v2) ;
                  libdatamsg(q330, LIBMSG_SEQBEG, (pointer)addr(s)) ; /* no continutity */
                end
              else
                begin
                  if ((q330->lastseq == 0) land (seqspread(dsn, paqs->dt_data_sequence) < 0) land
                    ...
                    then
                      begin
                        sprintf(s, "%d to %d", v1, v2) ;
                        libdatamsg (q330, LIBMSG_SEQOVER, (pointer)addr(s)) ;
                      end
                    else
                      begin
                        v3 = seqspread(dsn, paqs->dt_data_sequence) ;
                        sprintf(s, "%d, %d to %d", v3, v1, v2) ;
                        libdatamsg (q330, LIBMSG_SEQGAP, (pointer)addr(s)) ;
        ...
        then
          begin
            sprintf(s, "%d to %d", v1, v2) ;
            libdatamsg (q330, LIBMSG_SEQRESUME, (pointer)addr(s)) ;
          end
            ...
            then
                ...
                case DC_ST38 : /* already have the entire blockette */
                  switch (chan and not DCM_ST) begin
                    case ST38_GPSPH :
                      ...
                      libdatamsg(q330, LIBMSG_GPSSTATUS, (pointer)addr(s)) ;
                    ...
                    case ST38_DIGPH :
                      ...
                      libdatamsg(q330, LIBMSG_DIGPHASE, (pointer)addr(s)) ;
                      break ;
                    case ST38_BACKUP :
                      libdatamsg(q330, LIBMSG_SAVEBACKUP, (pointer)"") ;
                      break ;
                    case ST38_WINSTAT :
                      if (subchan == RWR_START)
                        then
                          libdatamsg(q330, LIBMSG_SCHEDSTART, (pointer)"") ;
                        else
                          begin
                            libdatamsg(q330, LIBMSG_SCHEDEND, (pointer)"") ;
#ifndef OMIT_SEED
                            flush_lcqs (paqs) ;
                            no_previous (paqs) ;
#endif
                      ...
                      break ;
                    case ST38_LEAP :
                      libdatamsg(q330, LIBMSG_LEAPDET, (pointer)"") ;
                      break ;
                  end
                  break ;
                case DC_ST816 :
                  switch (chan and not DCM_ST) begin
                    case ST816_PSPH :
                      libdatamsg (q330, LIBMSG_SMUPHASE, (pointer)pwrph[subchan]) ;
                      break ;
                    case ST816_AFAULT :
                      if (subchan != 0)
                        then
                          libdatamsg (q330, LIBMSG_APWRON, (pointer)"") ;
                        else
                          libdatamsg (q330, LIBMSG_APWROFF, (pointer)"") ;
                      break ;
                    case ST816_CALERR :
                ...
                case DC_ST32 :
                  ...
                  switch (chan and not DCM_ST) begin
                    case ST32_DRIFT :
                      ...
                      libdatamsg (q330, LIBMSG_PHASERANGE, (pointer)addr(s)) ;
                      break ;
                  end
                  break ;
                case DC_ST232 :
              ...
              switch (chan and DCM) begin
                              ...
                              then
                                begin
                                  ...
                                  if (lnot seqgap_occurred)
                                    then
                                      begin
                                        libdatamsg(q330, LIBMSG_TIMEJMP, (pointer)addr(s)) ;
#ifndef OMIT_SEED
                                        log_clock (q330, CE_JUMP, (pointer)addr(s)) ; /* any jump that big is logged! */
#endif
                                      end
                                  update_ok = FALSE ;
                                end
                                  ...
                                  if (abs(diff) >= q330->share.global.jump_thresh)
                                    then
                                      begin
                                        ...
                                        unlock (q330) ;
#ifndef OMIT_SEED
                                        log_clock (q330, CE_JUMP, (pointer)addr(s)) ;
#endif
                                        update_ok = FALSE ;
                                      end
                                    else
                      ...
                      if ((lnot paqs->daily_done) land (hr == 0))
                        then
                          begin
                            ...
                            log_clock (q330, CE_DAILY, (pointer)"") ;
                          end
                      else if (hr > 0)
                        then
                            ...
                            then
                              begin
                                if ((paqs->data_qual != paqs->last_data_qual) land
                                  ...
                                  then
                                    log_clock (q330, CE_VALID, (pointer)"") ;
                  ...
                  if ((wordval > MAXDATA) lor
                    ...
                    then
                      begin
                        v1 = wordval ;
                        sprintf(s, "%d", v1) ;
                        libdatamsg (q330, LIBMSG_INVBLKLTH, (pointer)addr(s)) ;
                        return ;
                      end
                  ...
                  switch (chan and not DCM) begin
                    ...
                    case SP_CFGBLK :
                      ...
                      switch (subchan) begin
                        case CFG_GLOBAL :
                          add_cfg (paqs, (pointer)"GL", p2, RAW_GLOBAL_SIZE, 0, 0) ;
                          break ;
                        case CFG_FIXED :
                          add_cfg (paqs, (pointer)"FX", p2, RAW_FIXED_SIZE, 0, 0) ;
                          break ;
                        case CFG_AUXAD :
                          add_cfg (paqs, (pointer)"XA", p2, CFGSZ_AUXAD, 0, 0) ;
                          break ;
                        case CFG_SS1 :
                          add_cfg (paqs, (pointer)"S1", p2, wordval - 4, 0, 0) ;
                          break ;
                        case CFG_SS2 :
                          add_cfg (paqs, (pointer)"S2", p2, wordval - 4, 0, 0) ;
                          break ;
                      end
#endif
                      break ;
                    ...
                    default :
                      ...
                      sprintf(s, "%X", v1) ;
                      zpad(s, 2) ;
                      libmsgadd(q330, LIBMSG_INVSPEC, (pointer)addr(s)) ;
                      break ;
                  end
                  if (skip)
                    then
                ...
                default :
                  ...
                  sprintf(s, "%X", v1) ;
                  zpad(s, 2) ;
                  libmsgadd(q330, LIBMSG_INVBLK, (pointer)addr(s)) ;
                  break ;
            end
        end
      break ;
