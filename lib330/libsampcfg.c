...
begin
  ...
  q330 = ownedby ;
  getthrbuf (q330, (pointer)addr(paqs), sizeof(taqstruc)) ;
  paqs->owner = ownedby ;
  clear_sg (paqs) ;
#ifndef OMIT_SEED
  getthrbuf (q330, (pointer)addr(msgq), sizeof(tmsgqueue)) ;
  paqs->msgqueue = msgq ;
  paqs->msgq_in = msgq ;
  paqs->msgq_out = msgq ;
  ...
  for (n = 2 ; n <= msgcnt ; n++)
    begin
      getthrbuf (q330, (pointer)addr(msgq->link), sizeof(tmsgqueue)) ;
      msgq = msgq->link ;
    end
...
begin
  ...
  switch (filt) begin
    case 0 :
      strcpy((char *)result, "Linear all") ;
      break ;
    case 1 :
      strcpy((char *)result, "Linear below 100sps") ;
      break ;
    case 2 :
      strcpy((char *)result, "Linear below 40sps") ;
      break ;
    case 3 :
      strcpy((char *)result, "Linear below 20sps") ;
      break ;
  end
  return (char *)result ;
end
                      ...
                      then
                        begin
                          ...
                          if ((show) land (q330->cur_verbosity and VERB_LOGEXTRA))
                            then
                              begin
                                sprintf(s, "%s@%s=%s", seed2string(addr(pchan->location), addr(pchan->seedname), (pchar)addr(s1)),
                                        scvrate(pchan->rate, (pointer)addr(s2)), realtostr(pchan->delay, 6, (pointer)addr(s4))) ;
                                libmsgadd (q330, LIBMSG_FILTDLY, (pointer)addr(s)) ;
                              end
                        end
                    break ;
                  end
  ...
  while (pl)
    begin
      ...
      if (pl->segsize)
        then
          begin
            getbuf (q330, (pointer *)addr(pl->segbuf), pl->segsize) ;
            pl->seg_next = pl->segbuf ;
            pl->seg_seq = 0xFFFFFFFF ;
            getbuf (q330, (pointer *)addr(pl->dholdq), sizeof(dholdqtype)) ;
            pl->dholdq->ppkt = NIL ;
                ...
                begin
                  ...
                  if (q330->cur_verbosity and VERB_LOGEXTRA)
                    then
                      begin
                         ;
                        sprintf(s, "%s:%d@%s,%s=%s", seed2string(addr(pl->location), addr(pl->seedname), (pchar)addr(s1)),
                                i + 1, scvrate(pl->rate, (pointer)addr(s2)),
                                sfcorner(q330, i, (pointer)addr(s3)), realtostr(pl->delay, 6, (pointer)addr(s4))) ;
                        libmsgadd (q330, LIBMSG_FILTDLY, (pointer)addr(s)) ;
                      end
                end
        ...
        then
          begin
            ...
            if (q330->cur_verbosity and VERB_LOGEXTRA)
              then
                begin
                  sprintf(s, "%s:%s@%s=%s", seed2string(addr(pl->location), addr(pl->seedname), (pchar)addr(s1)),
                          seed2string(addr(pl->prev_link->location), addr(pl->prev_link->seedname), (pchar)addr(s2)),
                          scvrate(pl->rate, (pointer)addr(s3)), realtostr(pl->delay, 6, (pointer)addr(s4))) ;
                  libmsgadd (q330, LIBMSG_FILTDLY, (pointer)addr(s)) ;
                end
...
begin
  ...
  getthrbuf (q330, (pointer *)addr(pr), sizeof(tcompressed_buffer_ring)) ;
  pr->full = FALSE ;
  pl->com->ring = pr ;
  ...
  if (q330->cur_verbosity and VERB_LOGEXTRA)
    then
      begin
        ...
        libdatamsg (q330, LIBMSG_TOTAL, (pointer)addr(s)) ;
      end
  flush_lcqs (paqs) ;
  if (q330->dssstruc)
    then
    ...
    begin
      ...
      while ((lcqstat->count < MAX_LCQ) land (q))
        begin
          ...
          strcpy((char *)addr(pone->location), (char *)addr(q->slocation)) ;
          strcpy((char *)addr(pone->channel), (char *)addr(q->sseedname)) ;
