          ...
          then
            ...
            begin
              ...
              if (q330->q330ip == INADDR_NONE)
                then
                  begin
                    libmsgadd(q330, LIBMSG_BADIPADDR, (pointer)"") ;
                    return TRUE ;
                  end
            end
      end
                  ...
                  if ((q330->libstate == LIBSTATE_RUNWAIT) lor (q330->libstate == LIBSTATE_RUN))
                    then
                      begin
                        ...
                        libmsgadd (q330, LIBMSG_ROUTEFAULT, (pointer)"Deallocating and waiting 10 minutes") ;
                      end
                    else
                      begin
...
begin
  ...
  p = (pbyte)addr(q330->commands.cmsgin.qdp) ;
    ...
    then
      begin
        ...
        if (err == SOCKET_ERROR)
          ...
          then
            tcp_error (q330, (pointer)"Connection Closed, Waiting 10 minutes") ; /* connection closed */
        else if (err > 0)
          then
            begin
              ...
              while (q330->tcpidx >= 4)
                begin
                  ...
                  if ((poff > 1) lor (qdplth < sizeof(tqdp)) lor
                    ...
                    then
                      begin
                        tcp_error (q330, (pointer)"Invalid header, Waiting 10 minutes") ;
                        return ;
                      end
