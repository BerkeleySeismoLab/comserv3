...
begin
  getbuf (pexp->q330, (pointer)addr(pexp->curpt), sizeof(tdetector_operation)) ;
...
begin
  ...
  tempq330 = paqs->owner ;
  getbuf (tempq330, (pointer)addr(pexp), sizeof(texpand)) ;
    ...
    then
      begin
        if (q->ctrl->logmsg)
          then
            begin
              strcpy(s, (char *)addr(q->ctrl->cdname)) ;
              if (q->ctrl->ison)
                then
                  strcat(s, " On") ;
                else
...
begin
  ...
  while ((ctrlstat->count < MAX_CTRLSTAT) land (pc))
    begin
      strcpy((char *)addr(ctrlstat->entries[ctrlstat->count].name), (char *)addr(pc->cdname)) ;
