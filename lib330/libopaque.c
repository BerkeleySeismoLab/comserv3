...
begin
  ...
  add_cfg (paqs, (pointer)"GL", addr(q330->raw_global), RAW_GLOBAL_SIZE, 0, 0) ;
  add_cfg (paqs, (pointer)"FX", addr(q330->raw_fixed), RAW_FIXED_SIZE, 0, 0) ;
  sprintf(s, "L%d", q330->par_create.q330id_dataport + 1) ;
  add_cfg (paqs, (pointer)addr(s), addr(q330->raw_log), RAW_LOG_SIZE, 0, 0) ;
  ...
  while (sizeleft > 0)
    begin
      ...
      add_cfg (paqs, (pointer)addr(s), pb, sz, rnum, flag) ;
      inc(rnum) ;
      incn(pb, sz) ;
