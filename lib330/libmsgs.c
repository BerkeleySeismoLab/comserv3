...
begin
  ...
  if (msgcode == 0)
    then
      s[0] = 0 ;
  strcpy((char *)addr(q330->msg_call.suffix), (char *)msgsuf) ;
  if (dt)
    then
      sprintf(s1, "[%s] ", jul_string(dt, (pchar)addr(s2))) ;
    else
      strcpy(s1, " ") ;
  sprintf(s, "{%d}%s%s%s", msgcode, s1, lib_get_msg (msgcode, (pointer)addr(s2)), (char *)msgsuf) ;
                ...
                then
                  begin
                    if (paqs->msgq_in->link != paqs->msgq_out)
                      then
                        begin
                          strcpy((char *)addr(paqs->msgq_in->msg), (char *)addr(s)) ;
...
begin
  sprintf((char *)result, "%d.%d.%d.%d", (integer)((num shr 24) and 255), (integer)((num shr 16) and 255),
