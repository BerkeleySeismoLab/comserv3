...
/* standard filter DEC10 */
  getthrbuf (q330, (pointer)addr(paqs->firchain), sizeof(tfilter)) ;
  ic = paqs->firchain ;
  ic->link = NIL ;
  strcpy((char *)addr(ic->fname), "DEC10") ; ;
  ic->len = 400 ;
  flen = 400 ;
  ic->gain = 1.0 ;
  ic->dec = 10 ;
...
/* standard filter VLP389 */
  getthrbuf (q330, (pointer)addr(ic->link), sizeof(tfilter)) ;
  ic = ic->link ;
  ic->link = NIL ;
  strcpy((char *)addr(ic->fname), "VLP389") ;
  ic->len = 389 ;
  ic->gain = 1.0 ;
  ic->dec = 10 ;
  ic->fir_num = 1 ;
...
/* standard filter ULP379 */
  ...
  ic = ic->link ;
  ic->link = NIL ;
  strcpy((char *)addr(ic->fname), "ULP379") ;
  ic->len = 379 ;
  ic->gain = 1.0 ;
  ic->dec = 10 ;
  ic->fir_num = 2 ;
...
begin
  ...
  while (src)
    begin
      getthrbuf (q330, (pointer)addr(dest->link), sizeof(tfilter)) ;
...
begin
  ...
  getbuf (q330, (pointer)addr(pf), sizeof(tfir_packet)) ;
  getbuf (q330, (pointer)addr(pf->fbuf), src->len * sizeof (tfloat)) ;
  pf->f = pf->fbuf ;
  pf->fcoef = (pointer)addr(src->coef) ;
  pf->flen = src->len ;
  pf->fdec = src->dec ;
  pf->fcount = 0 ;
...
begin
  ...
  if (points > 2)
    then
      ...
      extra = 0 ;
  getbuf (q330, (pointer)addr(pf), sizeof (tiirfilter) + extra) ;
