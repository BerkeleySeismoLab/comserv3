...
begin
  ...
  q330 = paqs->owner ;
  getbuf (q330, (pointer)addr(cur_iir), sizeof(tiirdef)) ;
...
begin
  ...
  ucs[len] = 0 ;
  lib330_upper((pchar)addr(ucs)) ;
  cur_fir = paqs->firchain ;
  while (cur_fir)
    begin
      strcpy(ucfname, cur_fir->fname) ;
      lib330_upper((pchar)addr(ucfname)) ;
...
begin
  ...
  q330 = paqs->owner ;
  getbuf (q330, (pointer)addr(cur_def), sizeof(tdetector)) ;
  ...
  for (i = 0 ; i <= 1 ; i++)
    if (q->location[i] != ' ')
      then
        s[lth++] = q->location[i] ;
  s[lth] = 0 ;
  strcpy((char *)addr(q->slocation), s) ;
  lth = 0 ;
  for (i = 0 ; i <= 2 ; i++)
    if (q->seedname[i] != ' ')
      then
        s[lth++] = q->seedname[i] ;
  s[lth] = 0 ;
  strcpy((char *)addr(q->sseedname), s) ;
end
...
begin
  ...
  q330 = paqs->owner ;
  getbuf (q330, (pointer)addr(cur_lcq), sizeof(tlcq)) ;
  if (paqs->lcqs == NIL)
    then
      paqs->lcqs = cur_lcq ;
    else
  ...
  cur_lcq->caldly = 60 ;
#ifndef OMIT_SEED
  getbuf (q330, (pointer)addr(cur_lcq->com), sizeof(tcom_packet)) ;
          ...
          if (pt->lcq_num == b)
            then
              begin
                ...
                getbuf (q330, (pointer)addr(pds), sizeof(tdownstream_packet)) ;
                pds->link = NIL ;
            ...
            while (pd)
              if (pd->detector_num == b)
                then
                  ...
                  pd = pd->link ;
            getbuf (q330, (pointer)addr(pdp), sizeof(tdet_packet)) ;
            pdp->detector_def = pd ;
...
begin
  ...
  q330 = paqs->owner ;
  getbuf (q330, (pointer)addr(cur_ctrl), sizeof(tcontrol_detector)) ;
  ...
  repeat
    ...
    if (b == (DES_OP or DEO_DONE))
      then
        break ;
    getbuf (q330, (pointer)addr(po), sizeof(tdop)) ;
    po->link = NIL ;
    po->tok = b ;
    po->point = NIL ;
...
begin
  ...
  while ((pntrint)*p < (pntrint)stop)
    begin
      ...
      if (b <= 31)
        then
          strncpy((char *)addr(paqs->commevents[b].name), (char *)addr(s), COMMLENGTH) ;
    end
end
...
begin
  ...
  loadstring (p, 8, (pointer)addr(pdss->high_pass)) ;
  loadstring (p, 8, (pointer)addr(pdss->mid_pass)) ;
  loadstring (p, 8, (pointer)addr(pdss->low_pass)) ;
...
begin
  ...
  for (i = 0 ; i <= CE_MAX - 1 ; i++)
    sprintf((char *)addr(paqs->commevents[i].name), "COMM:%d", i + 1) ;
  q = paqs->dplcqs ;
  while (q)
    begin
      q->validated = FALSE ;
      ...
      switch (tok) begin
        ...
        case TF_VERSION :
          ...
          if (tok != T_VER)
            then
              begin
                libmsgadd (q330, LIBMSG_INVTVER, (pointer)"") ;
        ...
        case T2_OPAQUE :
          ...
          getbuf (q330, (pointer)addr(paqs->opaque_buf), paqs->opaque_size) ;
  ...
  sprintf ((char *)addr(q330->station_ident), "%s-%s", s1, s) ; /* network-station */
  init_lcq (paqs) ;
#ifndef OMIT_SEED
  ...
  if (paqs->data_timetag > 1.0) /* if non-zero continuity was restored, write cfg blks at start of session */
    then
      begin
        libdatamsg (q330, LIBMSG_RESTCONT, (pointer)"") ;
        paqs->contingood = restore_continuity (q330) ;
        if (lnot paqs->contingood)
          then
            libdatamsg (q330, LIBMSG_CONTNR, (pointer)"") ;
      end
  ...
  if (seghdr.segnum == seghdr.segtotal)
    then
      begin
        ...
        libmsgadd (q330, LIBMSG_TOKREAD, (pointer)addr(s)) ;
...
begin
  ...
  libmsgadd (q330, LIBMSG_READTOK, (pointer)"") ;
  q330->data_timer = 0 ;
  pmem->start = 0 ;
  pmem->count = 0 ;
  pmem->memtype = MT_CFG1 + q330->par_create.q330id_dataport ;
