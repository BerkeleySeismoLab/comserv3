...
begin
  ...
  strncpy ((pointer)addr(pum->msg), (pointer)umsg, 79) ;
  ...
  if (q330->libstate == LIBSTATE_WAIT)
    then
      begin
        ...
        if (strcmp((pointer)addr(q330->par_register.q330id_address), (pointer)addr(newaddr)))
          then
            add_status (q330, AC_NEWIP, 1) ;
        strcpy((pointer)addr(q330->par_register.q330id_address), (pointer)addr(newaddr)) ;
        ...
        libmsgadd (q330, LIBMSG_POCRECV, (pointer)poc->log_info) ;
        q330->dynip_age = 0 ;
