...
begin
  ...
  if (desired < 1)
    then
      strcpy((char *)result, "Disabled ") ;
    else
      sprintf((char *)result, "%5.3f%% ", ((actual - desired) / desired) * 100.0) ;
  return (char *)result ;
end
...
begin
  ...
  libmsgadd(q330, LIBMSG_CAL, (pointer)"Digitizer Calibration Results:") ;
  s[0] = 0 ;
  for (i = 0 ; i <= 2 ; i++)
    begin
...
begin
  ...
  if (q330->share.gpsids[0][0])
    then
      begin
        libmsgadd(q330, LIBMSG_GPSIDS, (pointer)"GPS Engine Identification") ;
        log_nonblank(q330, (pointer)addr(q330->share.gpsids[0])) ;
        log_nonblank(q330, (pointer)addr(q330->share.gpsids[1])) ;
        log_nonblank(q330, (pointer)addr(q330->share.gpsids[2])) ;
        log_nonblank(q330, (pointer)addr(q330->share.gpsids[3])) ;
        log_nonblank(q330, (pointer)addr(q330->share.gpsids[4])) ;
        log_nonblank(q330, (pointer)addr(q330->share.gpsids[5])) ;
        log_nonblank(q330, (pointer)addr(q330->share.gpsids[6])) ;
        log_nonblank(q330, (pointer)addr(q330->share.gpsids[7])) ;
        log_nonblank(q330, (pointer)addr(q330->share.gpsids[8])) ;
      end
  ...
  libmsgadd(q330, LIBMSG_PLL, (pointer)"PLL Status") ;
        ...
        then
          begin
            if (strlen(m) >= 68)
              then
                begin
                  libmsgadd(q330, LIBMSG_TOTAL, (pointer)addr(m)) ;
                  strcpy(m, "written:") ;
                end
            sprintf(s, " %s-%d", seed2string((pointer)q->location, (pointer)q->seedname, (pchar)addr(s1)),
