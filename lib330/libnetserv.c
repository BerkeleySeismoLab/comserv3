...
begin
  ...
  if (nsstr->npath == INVALID_SOCKET)
    then
      begin
        ...
        lib_msg_add(nsstr->ns_par.stnctx, AUXMSG_SOCKETERR, 0, (pointer)addr(s)) ;
        return ;
      end
#ifndef X86_WIN32
  ...
  if (err)
    then
      begin
        ...
        nsstr->npath = INVALID_SOCKET ;
        sprintf(s, "%d on netserv[%d] port", err, nsstr->ns_par.server_number) ;
        lib_msg_add(nsstr->ns_par.stnctx, AUXMSG_BINDERR, 0, (pointer)addr(s)) ;
        return ;
      end
#ifdef X86_WIN32
  flag = 1 ;
  ...
  if (err)
    then
      begin
        ...
        nsstr->npath = INVALID_SOCKET ;
        sprintf(s, "%d on netserv[%d] port", err, nsstr->ns_par.server_number) ;
        lib_msg_add(nsstr->ns_par.stnctx, AUXMSG_LISTENERR, 0, (pointer)addr(s)) ;
        return ;
      end
  ...
  lib_msg_add(nsstr->ns_par.stnctx, AUXMSG_SOCKETOPEN, 0, (pointer)addr(s)) ;
  nsstr->sockopen = TRUE ;
end
        ...
        if ((err != EWOULDBLOCK) land (err != EINPROGRESS))
          then
            begin
              ...
              lib_msg_add(nsstr->ns_par.stnctx, AUXMSG_ACCERR, 0, (pointer)addr(s)) ;
            end
      end
    else
      begin
        ...
        lib_msg_add (nsstr->ns_par.stnctx, AUXMSG_CONN, 0, (pointer)addr(s)) ;
        lth = sizeof(integer) ;
          ...
          if ((err == ECONNRESET) lor (err == ECONNABORTED))
            then
              begin
                ...
                lib_msg_add(nsstr->ns_par.stnctx, AUXMSG_DISCON, 0, (pointer)addr(s)) ;
