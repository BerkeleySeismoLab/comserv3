...
#define MAX_DATE 0x7FFF0000 /* above this just has to be nonsense */
  ...
  strcpy((char *)addr(q330->miniseed_call.location), (char *)addr(q->slocation)) ;
  strcpy((char *)addr(q330->miniseed_call.channel), (char *)addr(q->sseedname)) ;
      ...
      begin
        ...
        if (loops > 20)
          then
            begin
              q330 = paqs->owner ;
              seed2string((pointer)q->location, (pointer)q->seedname, (pointer)addr(s)) ;
              libmsgadd (q330, LIBMSG_LBFAIL, (pointer)addr(s)) ;
            end
      end
...
begin
  ...
  phdr->dob.encoding_format = 11 ;
  p = (pbyte)addr(paqs->detcal_buf.rec) ;
  ...
  if (q->onesec_filter)
    then
      begin
        ...
        strcpy((char *)addr(q330->onesec_call.location), (char *)addr(q->slocation)) ;
        strcpy((char *)addr(q330->onesec_call.channel), (char *)addr(q->sseedname)) ;
...
begin
  ...
  if (paqs->data_timetag < 1)
    then
      begin
        libdatamsg (q330, LIBMSG_DISCARD, (pointer)seed2string((pointer)q->location, (pointer)q->seedname, (pchar)addr(s))) ;
...
begin
  ...
  q = paqs->proc_lcq ;
  pcmp = addr(q->precomp) ;
  seed2string((pointer)q->location, (pointer)q->seedname, (pchar)addr(s)) ;
  if (paqs->data_timetag < 1)
    then
      begin
        libdatamsg (q330, LIBMSG_DISCARD, (pointer)addr(s)) ;
        return ; /* don't know what time it is yet */
      end
  if (seqspread (paqs->dt_data_sequence, q->dtsequence) <= 0) /* has this second of data already been processed? */
    then
