...
#ifndef OMIT_SEED
  getthrbuf (q330, (pointer)addr(cur_lcq->com), sizeof(tcom_packet)) ;
  cur_lcq->com->frame = 1 ;
...
begin
  ...
  strcat(fname, "t") ;
  cf = lib_file_open (q330->par_create.file_owner, (pchar)addr(fname), LFO_OPEN or LFO_READ) ;
      ...
      begin
        ...
        repeat
          ...
          if (pdlsrc->crc != gcrccalc (addr(q330->crc_table), (pointer)((pntrint)pdlsrc + 4), pdlsrc->size - 4))
            then
              begin
                libmsgadd (q330, LIBMSG_CONCRC, (pointer)"Thread") ;
                lib_file_close (q330->par_create.file_owner, cf) ;
                if (paqs->msg_lcq == NIL)
                  then
                    build_fake_log_lcq (paqs, TRUE) ;
          ...
          if (pdlsrc->id != CTY_DPLCQ)
            then
              begin
                libmsgadd (q330, LIBMSG_CONPURGE, (pointer)"Thread") ;
                lib_file_close (q330->par_create.file_owner, cf) ;
                if (paqs->msg_lcq == NIL)
                  then
                    build_fake_log_lcq (paqs, TRUE) ;
...
begin
  ...
  strcpy(fname, q330->par_create.opt_contfile) ;
  strcat(fname, "q") ;
  cf = lib_file_open (q330->par_create.file_owner, (pchar)addr(fname), LFO_CREATE or LFO_WRITE) ;
  if (cf == INVALID_FILE_HANDLE)
    then
      begin
        q330->media_error = TRUE ;
...
begin
  ...
  repeat
    ...
    getthrbuf (q330, (pointer)addr(pcc), sz + sizeof(tcont_cache)) ;
    if (last)
      then
        last->next = pcc ;
      else
    ...
    if (hdr.crc != gcrccalc (addr(q330->crc_table), (pointer)((pntrint)pcc->payload + 4), sz - 4))
      then
        begin
          libmsgadd (q330, LIBMSG_CONCRC, (pointer)"Q330") ;
          lib_file_close (q330->par_create.file_owner, cf) ;
          return good ;
        end
      else
...
begin
  ...
  if (system.version != CT_VER)
    then
      begin
        ...
        libmsgadd (q330, LIBMSG_CONTIN, (pointer)addr(s)) ;
        return ;
      end
  ...
  if (system.id != CTY_SYSTEM)
    then
      begin
        libmsgadd (q330, LIBMSG_CONPURGE, (pointer)"Q330") ;
        return ;
      end
  ...
  if (system.reboot_counter != newreboots)
    then
      begin
        ...
        libmsgadd (q330, LIBMSG_CONTBOOT, (pointer)addr(s)) ;
    ...
    then
      begin
        libmsgadd (q330, LIBMSG_CONTBOOT, (pointer)"change in high frequency configuration") ;
        return ;
      end
  ...
  strcpy(fname, q330->par_create.opt_contfile) ;
  strcat(fname, "q") ;
  sprintf(s, "%d %s Q=%d", paqs->dt_data_sequence, realtostr(system.lasttime, 6, (pointer)addr(fname)), paqs->data_qual) ;
  libdatamsg (q330, LIBMSG_CONTFND, (pointer)addr(s)) ;
end
        ...
        case CTY_FIR :
          ...
          while (q)
            if ((memcmp(addr(q->location), addr(pfsrc->loc), 2) == 0) land
                ...
                (q->source_fir) land (strcmp((char *)addr(q->source_fir->fname), (char *)addr(pfsrc->fn)) == 0))
              then
                begin
...
begin
  ...
  libmsgadd (q330, LIBMSG_WRCONT, (pointer)"Q330") ;
  psystem = (pointer)q330->cbuf ;
  ...
  strcpy(fname, q330->par_create.opt_contfile) ;
  strcat(fname, "q") ;
  sprintf(s, "%d %s", paqs->dt_data_sequence, realtostr(psystem->lasttime, 6, (pointer)addr(fname))) ;
  libmsgadd (q330, LIBMSG_CSAVE, (pointer)addr(s)) ;
  bm = 0 ;
  for (i = 0 ; i <= CE_MAX - 1 ; i++)
    ...
    begin
      ...
      if ((q->fir) land (q->source_fir))
        then
          begin
            ...
            strcpy((char *)addr(pfdest->fn), (char *)addr(q->source_fir->fname)) ;
            pfdest->lpad = 0 ;
      ...
      while (psrc)
        begin
          ...
          strcpy((char *)addr(pdest->fn), (char *)addr(psrc->def->fname)) ;
          pdest->lpad = 0 ;
          ...
          if (pdp->detector_def->dtype == MURDOCK_HUTT)
            then
              begin
                ...
                strcpy((char *)addr(pmdest->dn), (char *)addr(pdp->detector_def->detname)) ;
                pmdest->lpad = 0 ;
                pmc = pdp->cont ;
              ...
              begin
                ...
                strcpy((char *)addr(ptdest->dn), (char *)addr(pdp->detector_def->detname)) ;
                ptdest->lpad = 0 ;
                pmt = pdp->cont ;
...
begin
  ...
  cf = lib_file_open (q330->par_create.file_owner, (pchar)addr(fname), LFO_OPEN or LFO_READ or LFO_WRITE) ;
  if (cf == INVALID_FILE_HANDLE)
    then
      return ;
  pdlsrc = (pointer)q330->cbuf ;
