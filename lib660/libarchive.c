...
begin
  ...
  memclr(addr(parc->hdr_buf), sizeof(seed_header)) ;
end
...
#define MAX_DATE 0x7FFF0000 /* above this just has to be nonsense */
  ...
  storeseedhdr (addr(p), (pvoid)addr(parc->hdr_buf), q->pack_class == PKC_DATA) ; /* make sure is current */
  ...
  if (q660->par_create.call_aminidata)
    then
      q660->par_create.call_aminidata (addr(q660->miniseed_call)) ;
  if (parc->leave_in_buffer)
    then
  ...
  switch (q->pack_class) begin
      ...
      if (parc->total_frames > 1)
        then
          begin /* check for gaps */
            ...
            if (((bcnt + fcnt + parc->total_frames) > q660->arc_frames) lor (fabs(tdiff) > q->gap_secs))
    ...
    case PKC_MESSAGE :
      if (pbuf->hdr_buf.samples_in_record == 0)
        then
          return ;
      if (((pbuf->hdr_buf.samples_in_record + parc->hdr_buf.samples_in_record) > (q660->arc_size - NONDATA_OVERHEAD)) lor
                  ...
                  if (q660->miniseed_call.miniseed_action == MSA_RETARC)
                    then
                      begin /* extract seed header and set flags */
                        p = (pointer)parc->pcfr ;
                        loadseedhdr (addr(p), addr(parc->hdr_buf), (q660->miniseed_call.packet_class == PKC_DATA)) ;
                        fcnt = parc->hdr_buf.deb.frame_count + parc->hdr_buf.number_of_following_blockettes - 1 ;
                        if ((q->pack_class == PKC_DATA) land (fcnt < q660->arc_frames) land (q660->contingood))
                          then
                            begin /* try to extend */
