...
void load_firfilters (pq660 q660)
{
    pfilter ic ;
    int i, cnt ;

    /* standard filter VLP389 */
    getbuf (&(q660->thrmem), (pvoid)&(q660->firchain), sizeof(tfilter)) ;
    ic = q660->firchain ;
    ic->link = NIL ;
    strcpy(ic->fname, "VLP389") ;
    ic->len = 389 ;
    ic->gain = 1.0 ;
    ic->dec = 10 ;
    cnt = (int)ic->len ;
    ...
    getbuf (&(q660->thrmem), (pvoid)&(ic->link), sizeof(tfilter)) ;
    ic = ic->link ;
    ic->link = NIL ;
    strcpy(ic->fname, "ULP379") ;
    ic->len = 379 ;
    ic->gain = 1.0 ;
    ic->dec = 10 ;
    cnt = (int)ic->len ;
...
void append_firfilters (pq660 q660, pfilter src)
{
    pfilter dest ;
    ...
    while (src) {
        getbuf (&(q660->thrmem), (pvoid)&(dest->link), sizeof(tfilter)) ;
...
static tfloat scalar_product (tvector *a, tvector *b, int vector_length, int offset)
{
    int counter ;
...
static void time_shift (tvector *filter_history, int length, int shift)
{
    int index ;
...
static double recursive_filter (tiirsection *f, double s)
{
    ...
    f->x[0] = s ;
    f->y[0] = scalar_product (&(f->x), &(f->a), f->poles+1, 0)
              + scalar_product (&(f->y), &(f->b), f->poles, 1) ;
    time_shift (&(f->x), f->poles + 1, 1) ;
    time_shift (&(f->y), f->poles + 1, 1) ;
    return f->y[0] ;
}
...
double multi_section_filter (piirfilter resp, double s)
{
    ...
    if (resp->sects > 0)
        for (sect = 1 ; sect <= resp->sects ; sect++)
            s = recursive_filter (&(resp->filt[sect]), s) ;
...
static double factorial (int npoles, int index)
{
    int i, j, nmi ;
...
void bwsectdes (pdouble a, pdouble b, int npoles, BOOLEAN high,
                tfloat ratio)
{
    double p1[6], p2[6] ;
...
void calc_section (tsect_base *sect)
{
    ...
    bwsectdes (&(a_coef[1]), &(b_coef[1]), sect->poles, sect->highpass, sect->ratio) ;
...
pfilter find_fir (pq660 q660, pchar name)
{
    pfilter src ;
...
pfir_packet create_fir (pq660 q660, pfilter src)
{
    pfir_packet pf ;

    getbuf (&(q660->connmem), (pvoid)&(pf), sizeof(tfir_packet)) ;
    getbuf (&(q660->connmem), (pvoid)&(pf->fbuf), src->len * sizeof (tfloat)) ;
    pf->f = pf->fbuf ;
    pf->fcoef = (pvoid)&(src->coef) ;
    pf->flen = src->len ;
    pf->fdec = src->dec ;
    pf->fcount = 0 ;
...
piirfilter create_iir (pq660 q660, piirdef src, int points)
{
    piirfilter pf ;
    U16 extra ;
    int i, j ;
    ...
    getbuf (&(q660->connmem), (pointer)&(pf), sizeof (tiirfilter) + extra) ;
    pf->link = NIL ;
    pf->def = src ;
    pf->sects = src->sects ;

    for (i = 1 ; i <= pf->sects ; i++) {
        /* Copy in info from source for this section */
        pf->filt[i].poles = src->filt[i].poles ;
        pf->filt[i].highpass = src->filt[i].highpass ;
        pf->filt[i].ratio = src->filt[i].ratio ;
        pf->filt[i].spare = src->filt[i].spare1 ;
        ...
        bwsectdes (&(a_coef[1]), &(b_coef[1]), pf->filt[i].poles, pf->filt[i].highpass, pf->filt[i].ratio) ;

        for (j = 0 ; j <= pf->filt[i].poles ; j++) {
            if (j == 1)
                pf->filt[i].a[j] = a_coef[j + 1] * src->gain ;
            else
                pf->filt[i].a[j] = a_coef[j + 1] ;

            pf->filt[i].b[j] = b_coef[j + 1] ;
            pf->filt[i].x[j] = 0.0 ;
            pf->filt[i].y[j] = 0.0 ;
        }
    }
...
void allocate_lcq_filters (pq660 q660, plcq q)
{
    int points ;
    pdet_packet pdp ;
    piirfilter pi ;
    ...
    while (pdp) {
        ...
        if (pdp->detector_def->detfilt) {
            ...
            if (pi == NIL) {
                pi = create_iir (q660, pdp->detector_def->detfilt, points) ;

                if (pi)
                    q->stream_iir = extend_link (q->stream_iir, pi) ;
            }

            initialize_detector (q660, pdp, pi) ;
        } else
            initialize_detector (q660, pdp, NIL) ;

        pdp = pdp->link ;
    }

    if (q->source_fir)
        q->fir = create_fir (q660, q->source_fir) ;
}
