...
/* standard filter VLP389 */
  getbuf (addr(q660->thrmem), (pvoid)addr(q660->firchain), sizeof(tfilter)) ;
  ic = q660->firchain ;
  ic->link = NIL ;
...
/* standard filter ULP379 */
  getbuf (addr(q660->thrmem), (pvoid)addr(ic->link), sizeof(tfilter)) ;
  ic = ic->link ;
  ic->link = NIL ;
...
begin
  ...
  while (src)
    begin
      getbuf (addr(q660->thrmem), (pvoid)addr(dest->link), sizeof(tfilter)) ;
...
begin
  ...
  getbuf (addr(q660->connmem), (pvoid)addr(pf), sizeof(tfir_packet)) ;
  getbuf (addr(q660->connmem), (pvoid)addr(pf->fbuf), src->len * sizeof (tfloat)) ;
  pf->f = pf->fbuf ;
  pf->fcoef = (pvoid)addr(src->coef) ;
  pf->flen = src->len ;
  pf->fdec = src->dec ;
  pf->fcount = 0 ;
...
piirfilter create_iir (pq660 q660, piirdef src, integer points)
begin
  piirfilter pf ;
  word extra ;
  integer i, j ;
  ...
  if (points > 2)
    then
      ...
      extra = 0 ;
  getbuf (addr(q660->connmem), (pointer)addr(pf), sizeof (tiirfilter) + extra) ;
  pf->link = NIL ;
  pf->def = src ;
  pf->sects = src->sects ;
  for (i = 1 ; i <= pf->sects ; i++)
    begin
      ...
      bwsectdes (addr(a_coef[1]), addr(b_coef[1]), pf->filt[i].poles, pf->filt[i].highpass, pf->filt[i].ratio) ;
